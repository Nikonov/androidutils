...
package com.appmobileos.android.utils.db;
...
public class SelectionBuilder {
    ...
    private SelectionBuilder where(String selection, String operation, String... selectionArgs) {
        ...
        if (mSelection.length() > 0) {
            mSelection.append(" ").append(operation).append(" ");
        }
        ...
        logDebug(TAG, " Create selection = " + mSelection.toString() + " selectionArgs = " + selectionArgs.length);
    ...
    public SelectionBuilder where(String selection) {
    ...
    /**
     * Append the given selection clause to the internal state. Each clause is
     * surrounded with parenthesis and combined using {@code OR}.
     */
    public SelectionBuilder whereOR(String selection, String... selectionArgs) {
        return where(selection, "OR", selectionArgs);
    }
    ...
    public SelectionBuilder map(String table, String fromColumn, String toClause) {
        mProjectionMap.put(fromColumn, table + "." + fromColumn + " AS " + toClause);
        return this;
    }
    ...
    public Cursor query(SQLiteDatabase db, boolean distinct, String[] columns, String orderBy,
        ...
        if (isDebugGradleBuildEnable()) {
            String sql = SQLiteQueryBuilder.buildQueryString(distinct, mTable, columns, getSelection(), mGroupBy,
                    mHaving, orderBy, limit);
            logDebug(TAG, " Query: " + sql);
        }
